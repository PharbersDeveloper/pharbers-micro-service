...
trait one2manyTrait {
    ...
    val queryCondition: List[Map[String, JsValue]] => JsValue
    ...
    def queryConnection(data: JsValue, primary_key: String = "_id")
        ...
        val ta = pr match {
            ...
            case Some(ts) =>
                ...
                db.queryMultipleObject(bind_qc(ts), connect)(bind_ssr) match {
                    case Nil => Nil
                    case lst: List[Map[String, JsValue]] =>
                        val ta_module: phForward = new phForward { override lazy val module_name: String = ta_name }
                        val ta_result = ta_module.forward(s"/api/$ta_name/query/multi").post(queryCondition(lst))
